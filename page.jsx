...
export default function ConnectedFinancialAdvisor() {
  ...
  const loadData = useCallback(async (currentHousehold) => {
    ...
    const [{ data: accountRows }, { data: transactionRows }, { data: ruleRows }, { data: goalRows }, { data: enrollmentRows }] = await Promise.all([
      ...
      supabase.from("plaid_items").select("*").eq("household_id", currentHousehold.id).order("created_at", { ascending: true }),
    ]);
  ...
  const { open: openPlaid, ready: plaidReady } = usePlaidLink({
    ...
    onSuccess: async (publicToken, metadata) => {
      ...
      const data = await res.json();
      if (!res.ok) { setMessage(data.error || "Could not save connection."); return; }
      setMessage(`Connected ${data.institution_name}. Syncing...`);
      setLinkToken(null);
  ...
  async function disconnectBank(itemId, institutionName) {
    if (!confirm(`Disconnect ${institutionName}? This will remove all its accounts and transactions.`)) return;
    setIsBusy(true);
    setMessage(`Disconnecting ${institutionName}...`);

    const supabase = getBrowserSupabase ? null : null;
    const admin = await fetch("/api/plaid/disconnect", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ itemId, userId: user.id, householdId: household.id }),
    });

    const data = await admin.json();
    if (!admin.ok) { setMessage(data.error || "Could not disconnect."); setIsBusy(false); return; }
                ...
                {plaidItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div>
                        <p className="font-black">{item.institution_name}</p>
                        <p className="text-sm text-slate-500">{itemAccounts.length} account{itemAccounts.length !== 1 ? "s" : ""} • Connected {new Date(item.created_at).toLocaleDateString()}</p>
                      </div>
                      <Button
                        variant="ghost"
                        onClick={() => disconnectBank(item.item_id, item.institution_name)}
                        disabled={isBusy}
